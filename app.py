...
def run_analysis(ml, im, threshold=2):
    ...
    for inf, aud in sorted_inf:
        new = len(aud - cum)
        marginal.append((inf, new))
        cum |= aud
    results['marginal_gains'] = marginal
...
col1.metric("Unique Audience", results['total_unique_audience'])
col2.metric("Audience Estimate", aud_est)
col3.metric("Core Users Estimate", core_est)
col4.metric("Core %", f"{results['core_percentage']:.2f}%")
...
# ----------------------------
# Campaign Planner (with per-row exclusivity)
# ----------------------------
st.header("Campaign Planner")
st.subheader("Influencer Network Table")
st.caption("Select to include/exclude, then click **Update Selections**.")
...
with st.form("campaign_selection"):
    edited = st.data_editor(
        ...
        column_config={
            "Include in Network": st.column_config.CheckboxColumn(
                "Include",
                disabled=lambda row: row["Exclude from Analysis"]
            ),
            "Exclude from Analysis": st.column_config.CheckboxColumn(
                "Exclude",
                disabled=lambda row: row["Include in Network"]
            )
        },
...
if submitted:
    ...
    # enforce mutual exclusivity one more time
...
sel = df[df['Include in Network'] & ~df['Exclude from Analysis']]

total_impr      = int(sel['median_est_view_count'].sum())
total_reach     = int(sel['user_reach'].sum())
total_core_reach= int(sel['core_users_reached'].sum())

m1, m2, m3, m4 = st.columns(4)
m1.metric("Total Impressions", f"{total_impr:,}")
m2.metric("Reach Estimate", f"{total_reach:,}")
m3.metric("Core Impressions", f"~{int(total_impr * 0.3):,}")
m4.metric("Core Reach", f"{total_core_reach:,}")
